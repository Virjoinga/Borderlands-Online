...
[AddComponentMenu("uniSWF/MovieClipBehaviour")]
[ExecuteInEditMode]
public class MovieClipBehaviour : PumpkinBehaviour
{
	...
	protected virtual IGraphicsGenerator instanceGfxGenerator()
	{
		...
		graphicsGenerator.meshGeneratorOptions.doubleBuffered = doubleBufferMesh;
		return graphicsGenerator;
	}

	public override void OnUpdate()
	{
		if (Application.isEditor && editorPreview && ((lastMovieClip != null && swf != null && lastMovieClip.CompareTo(swf) != 0) || (lastSymbolName != null && symbolName != null && lastSymbolName.CompareTo(symbolName) != 0)))
		{
			Awake();
		}
		...
		else if (drawMeshMode && _enableRender && m_LastRenderMesh != null && (bool)m_LastRenderMesh.mesh)
		{
			for (int i = 0; i < m_LastRenderMesh.mesh.subMeshCount; i++)
			{
				...
				matrix4x.SetTRS(m_TmpVector, Quaternion.identity, Vector3.one);
				UnityEngine.Graphics.DrawMesh(m_LastRenderMesh.mesh, localToWorldMatrix * matrix4x, m_LastRenderMesh.materials[i], base.gameObject.layer, null, i);
			}
		}
	}
