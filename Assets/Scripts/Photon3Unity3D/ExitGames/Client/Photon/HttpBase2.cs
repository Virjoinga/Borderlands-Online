...
namespace ExitGames.Client.Photon
{
	internal class HttpBase2 : PeerBase
	{
		...
		internal override void Disconnect()
		{
			...
			Request(new byte[1] { 1 }, UrlParameters, true);
		}
		...
		{
			if (peerConnectionState == ConnectionStateValue.Connected && SupportClass.GetTickCount() - lastPingTimeStamp > timePingInterval)
			{
				lastPingTimeStamp = SupportClass.GetTickCount();
		...
		{
			Request(data, urlParamter, false);
		}
		...
		internal bool CheckResult()
		{
			...
			{
				ReceiveIncomingCommands(wWW.bytes, wWW.bytes.Length);
			}
			return true;
		}
	}
